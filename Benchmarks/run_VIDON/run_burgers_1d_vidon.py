...
def setup_logging(custom_log_dir=None):
    ...
    if custom_log_dir:
        ...
        model_folder_name = "VIDON_burgers_1d"
...
def create_vidon_model(args, device):
    """Create VIDON model for Burgers 1D problem."""
    print(f"\n--- Initializing VIDON Model for Burgers 1D ---")
...
def main():
    ...
    if not model_was_loaded:
        ...
        print(f"\nVIDON Burgers 1D model loaded. Skipping training.")
    ...
    # Save model
    if not model_was_loaded:
        model_save_path = os.path.join(log_dir, "burgers1d_vidon_model.pth")
    ...
    save_experiment_configuration(args, vidon_model, dataset, dataset_wrapper=data_generator, device=device, log_dir=log_dir, dataset_type="burgers_1d", test_results=test_results)
