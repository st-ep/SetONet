...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Train SetONet for OT transport-plan prediction.')
    ...
    parser.add_argument(
        ...
        help='Branch head type: standard (pool+rho), petrov_attention (Petrov-Galerkin attention), '
             'galerkin_pou (Galerkin partition-of-unity), quadrature (additive quadrature over learned '
             'test functions), or adaptive_quadrature (input-adaptive quadrature).',
    )
...
def setup_logging(project_root, custom_log_dir=None):
    ...
    if custom_log_dir:
        ...
        model_folder_name = 'SetONet_transport_plan'
...
def create_model(args, device, output_size_tgt):
    """Create SetONet model for transport-plan prediction."""
...
def run_training(args, device, log_dir):
    ...
    # Save model
    model_save_path = os.path.join(log_dir, 'transport_plan_setonet_model.pth')
