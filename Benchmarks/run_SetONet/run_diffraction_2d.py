...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Train SetONet for 2D diffraction problem.")
    ...
    parser.add_argument('--son_aggregation', type=str, default="attention", choices=["mean", "attention", "sum"],
                        help='Aggregation type for SetONet')
    parser.add_argument('--activation_fn', type=str, default="relu",
                        choices=["relu", "tanh", "gelu", "swish"], help='Activation function for SetONet networks')
    parser.add_argument(
        '--son_branch_head_type',
        type=str,
        default="standard",
        choices=["standard", "petrov_attention", "galerkin_pou", "quadrature", "adaptive_quadrature"],
        help="Branch head type: standard (pool+rho), petrov_attention (Petrov-Galerkin attention), "
             "galerkin_pou (Galerkin partition-of-unity), quadrature (additive quadrature over learned test "
             "functions), or adaptive_quadrature (input-adaptive quadrature).",
    )
    parser.add_argument('--son_pg_dk', type=int, default=None,
                        help='PG attention key/query dim (default: son_phi_output_size)')
    parser.add_argument('--son_pg_dv', type=int, default=None,
                        help='PG attention value dim (default: son_phi_output_size)')
    parser.add_argument(
        '--son_pg_no_logw',
        action='store_true',
        help='Disable adding log(sensor_weights) to PG attention logits (weights are unused by default).',
    )
    parser.add_argument('--son_galerkin_dk', type=int, default=None,
                        help='Galerkin PoU key/query dim (default: son_phi_output_size)')
    parser.add_argument('--son_galerkin_dv', type=int, default=None,
                        help='Galerkin PoU value dim (default: son_phi_output_size)')
    parser.add_argument('--son_quad_dk', type=int, default=64,
                        help='Quadrature/adaptive quadrature key/query dim (default: 64)')
    ...
    parser.add_argument(
        ...
        help='Galerkin PoU normalization: "none" (no norm), "total" (divide by total weight), '
             '"token" (per-token mass norm).',
    )
    parser.add_argument(
    ...
    parser.add_argument('--son_adapt_quad_hidden', type=int, default=64,
                        help='Adaptive quadrature adapter MLP hidden dimension')
    parser.add_argument('--son_adapt_quad_scale', type=float, default=0.1,
                        help='Adaptive quadrature tanh-bounded update scale')
    ...
    parser.add_argument('--pos_encoding_max_freq', type=float, default=0.01,
                        help='Max frequency for sinusoidal positional encoding')
...
def setup_logging(project_root, custom_log_dir=None):
    ...
    if custom_log_dir:
        ...
        model_folder_name = "SetONet_diffraction2d"
...
def create_model(args, device):
    """Create SetONet model for 2D diffraction problem."""
...
def main():
    ...
    if not model_was_loaded:
        model_save_path = os.path.join(log_dir, "diffraction2d_setonet_model.pth")
