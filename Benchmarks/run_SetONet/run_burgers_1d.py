...
def setup_logging(custom_log_dir=None):
    ...
    if custom_log_dir:
        ...
        model_folder_name = "SetONet_burgers_1d"
...
def create_setonet_model(args, device):
    """Create SetONet model for Burgers 1D problem."""
    print(f"\n--- Initializing SetONet Model for Burgers 1D ---")
    ...
    model = SetONet(
        ...
        output_size_src=1,  # Scalar initial condition values
        input_size_tgt=1,  # 1D coordinates (x)
        output_size_tgt=1,  # Scalar solution values
        p=args.son_p_dim,
...
def main():
    ...
    if not model_was_loaded:
        ...
        print(f"\nSetONet Burgers 1D model loaded. Skipping training.")
    ...
    # Save model
    if not model_was_loaded:
        model_save_path = os.path.join(log_dir, "burgers1d_setonet_model.pth")
    ...
    save_experiment_configuration(args, setonet_model, dataset, dataset_wrapper=data_generator, device=device, log_dir=log_dir, dataset_type="burgers_1d", test_results=test_results)
